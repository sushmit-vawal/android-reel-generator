...
class ContentCandidatePlannerTest {
    ...
    @Test fun allFourStructuresPreserveExactWording() = runTest {
        for (texts in listOf(listOf("The ocean can wait."), listOf("The ocean can wait.", "We have all afternoon."))) {
            for (count in 1..2) {
                ...
                val plan = ContentCandidatePlanner().select(request().copy(sources = listOf(clip, second)), listOf(item(texts, count)), listOf(video, video.copy(uri = second.uri)), emptyList(), emptyMap())
    ...
    @Test fun categoriesDisabledHistoryAndUnmatchedFootageExcludeImports() = runTest {
        ...
        for (excluded in listOf(original.copy(category = "Motivation"), original.copy(enabled = false), original.copy(tags = "", beatsJson = "[\"Airport check in.\"]", subTheme = "flight"))) {
            val plan = planner.select(request(), listOf(excluded), listOf(video), emptyList(), emptyMap())
            assertFalse(plan.metadata.notes.contains("contentItemId=item"))
        }
        val plan = planner.select(request(), listOf(original), listOf(video), listOf("The ocean can wait!"), emptyMap())
        assertFalse(plan.metadata.notes.contains("contentItemId=item"))
    }
    ...
    @Test fun importSelectPublishReopenPreventsReuseAndCountsExactlyOnce() = runTest {
        ...
        try {
            ...
            val plan = ContentCandidatePlanner().select(request(), db.dao().contentItems(), listOf(video), db.dao().completedCaptions(), emptyMap())
            ...
            val next = ContentCandidatePlanner().select(request().copy(id = "next"), db.dao().contentItems(), listOf(video), db.dao().completedCaptions(), emptyMap())
    ...
    @Test fun modelProviderCandidatesParticipateWithoutBeingRelabeledFallback() = runTest {
        ...
        val plan = planner.select(request(), emptyList(), listOf(video), (0..4).map { PhaseTwoText.caption(ReelCategory.TRAVEL, HumorStyle.AUTO, it, 12) }, emptyMap())
    ...
    @Test fun insufficientFootageDoesNotFlattenOrTruncateScript() = runTest {
        ...
        val plan = ContentCandidatePlanner().select(request(), listOf(item(texts)), listOf(video), emptyList(), emptyMap())
        assertFalse(plan.metadata.notes.contains("contentItemId=item"))
    }
}
