...
class ReelDatabaseTest {
    ...
    @Test fun migrationRetainsPhaseTwoFoldersSettingsAndCompletedReels() = runTest {
        ...
        db = Room.databaseBuilder(context, ReelDatabase::class.java, "test.db").addMigrations(ReelDatabase.MIGRATION_1_2, ReelDatabase.MIGRATION_2_3, ReelDatabase.MIGRATION_3_4, ReelDatabase.MIGRATION_4_5).allowMainThreadQueries().build()
        assertEquals("Trips", db.dao().folders().single().name)
