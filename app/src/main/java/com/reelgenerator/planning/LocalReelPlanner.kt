...
data class PlanningRequest(
    ...
    val scriptBeats: List<String>? = null,
...
class LocalReelPlanner : ReelPlanner {
    override fun plan(request: PlanningRequest): ReelPlan {
        ...
        var texts = request.scriptBeats ?: if (multipleBeats) lines else listOf(caption)
        require(texts.size in 1..6 && texts.all { it.isNotBlank() && it.length <= 180 }) { "Script needs 1–6 beats of at most 180 characters each." }
        var readingMs = texts.sumOf { ReadingDuration.minimumMs(it) }
        val desiredDuration = maxOf(8_000L, readingMs + 1_000).coerceAtMost(if (request.scriptBeats != null) 20_000 else 15_000)
        var wantedClips = if (request.forceSingle || request.slot < 2) 1 else if (request.slot == 2) 3 else 2
        if (!request.forceSingle && request.preferredClipCount != null) wantedClips = request.preferredClipCount.coerceIn(1, 5)
        if (!request.forceSingle && primary.durationMs < readingMs && request.preferredClipCount == null) wantedClips = 5
        ...
        if (duration < readingMs && multipleBeats && request.scriptBeats == null) {
